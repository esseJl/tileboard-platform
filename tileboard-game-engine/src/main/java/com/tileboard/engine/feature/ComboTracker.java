...
public final class ComboTracker {
    ...
    public void setComboTimeout(long millis) { this.timeoutMillis = millis; }

    /** Records a hit. Auto-resets if the combo timeout elapsed since the last hit. */
    public int hit() {
        long now = System.currentTimeMillis();
        if (lastHit.get() > 0 && now - lastHit.get() > timeoutMillis) {
            combo.set(0);
        }
        lastHit.set(now);
    ...
    public void reset() { combo.set(0); }

    public int current() { return combo.get(); }

    public int max()     { return maxCombo.get(); }
